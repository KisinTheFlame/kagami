...
pnpm app:deploy <agent|console|gateway|oss|browser|llm|metric|spire|napcat>  # 单服务：只重建重载该服务，不跑迁移、不动其它进程

pnpm app:stop                          # 停掉 ecosystem 里全部进程
pnpm app:stop <agent|console|gateway|oss|browser|llm|metric|spire|napcat>    # 只停该服务（与 app:deploy 共用同一套短名别名）
```
...
- `pnpm app:deploy <服务名>` = 单服务：只重建重载该服务。改单个服务时优先用它——重载 `console` / `gateway` / `browser` / `llm` / `metric` / `spire` / `napcat` 不会打断 `kagami-agent` 的热状态（KV 缓存前缀、HNSW 索引、活内存），符合 KV 缓存优先。（`web` 是 `gateway` 的已弃用别名。）
