...
- System prompt 可通过文件轻松修改，支持思考链指导
...
2. **提示词文件**：确保 `static/prompt.txt` 文件存在且包含思考链格式指导
3. **网络延迟**：LLM 请求可能有一定延迟，思考链会增加少量处理时间
4. **API 限制**：注意 LLM API 的频率限制和配额管理，主动模式可减少不必要调用
5. **日志管理**：思考链会产生较多日志输出，注意日志存储和轮转策略
6. **内容审核**：建议对思考过程和回复内容都添加过滤以确保安全合规
7. **向后兼容**：部署时新旧格式会并存，确保历史数据正常处理
8. **类型兼容**：确保所有依赖的 node-napcat-ts 版本兼容