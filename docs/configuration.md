...
- 顶层 `services`（与 `server` 平级）：各服务监听端口与地址的唯一事实来源，`services.{agent,console,gateway,oss,browser,llm,metric,spire,napcat}.{host,port}`，所有进程读它寻址；`services.{oss,browser,llm,metric,spire,napcat}` 仅 localhost。
