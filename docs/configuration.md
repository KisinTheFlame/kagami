...
- `server.llm.usages.{agent,vision}`（usage = KV 缓存身份，可配的仅这两个。fork 型 task agent `contextSummarizer` / `todoSuggestionAgent` / `innerVoice` 在调用点用 `usage=agent` 复用主 Agent 前缀命中 prompt cache，不单独配置；调用归因走 `scene` 自由字段，见 #555）
...
- 独立服务库（epic #539「每个持库服务独立数据库」，均与主库同目录不同文件）：scheduler 独占 `data/sqlite/scheduler.db`（`services.scheduler.databaseUrl`，#493）；napcat 独占 `data/sqlite/napcat.db`（`services.napcat.databaseUrl`，#539，schema 在 `apps/napcat/prisma/`）；llm 独占 `data/sqlite/llm.db`（`services.llm.databaseUrl`，#539，schema 在 `apps/llm/prisma/`）。metric 独占 `data/metric/metric.duckdb`（#475）、oss 独占 `data/oss/oss.db`。主库 kagami.db 由 agent 独占（console #539 起零 DB、经各服务查询路由聚合）。
...
独立库服务（scheduler / napcat / llm）的迁移走各自包内的同名脚本（同一 `scripts/prisma.sh` 参数化复用），如 `pnpm --filter @kagami/napcat db:migrate:dev -- --name <name>`；`pnpm app:deploy` 的 Step 2b/2c/2d 会分别应用，且各只停对应单进程。
...
- **主库 VACUUM**（一次性/低频）：DROP 大表后物理空间不会自动回收（页只进 freelist）。VACUUM 需要对库独占且不能在事务内，操作序：`pm2 stop kagami-agent` → `sqlite3 data/sqlite/kagami.db "VACUUM;"` → `pm2 start kagami-agent`。#539 收尾 DROP 九张旧表后已执行过一次。
