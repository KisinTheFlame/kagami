...
- refactor(oss): OSS 服务进程 put/get 全面流式化，大对象不再整体驻留内存（[#190](https://github.com/KisinTheFlame/kagami/issues/190)）。`put` 改为消费请求流：锁外边流边算 sha256 落 `blobs/tmp/` 临时文件、锁内只做 rename + 事务，峰值内存从 O(对象大小) 降到 O(chunk)，慢速大上传不再占用写锁串行阻塞其它 put/delete；`get` 改为 `open` fd（缺失→抛→500，fd 打开后免疫并发 unlink）后 `createReadStream(autoClose)` 直接 pipe 到响应，content-length 取自 db `blob.size`。超限（`maxBytes`）路径先 `writeHead(413)` 再收尾，客户端拿到真实 413 而非 ECONNRESET。临时文件挪到专用 `blobs/tmp/` 子目录，`sweepOrphans` 兼收崩溃残留。仅改 OSS 服务进程，`HttpOssClient` 与 agent 侧 base64 消费端不动。
