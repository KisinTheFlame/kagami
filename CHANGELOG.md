...
- build/lint: ESLint 开启类型感知 linting（`typescript-eslint` `recommendedTypeChecked` + `projectService`，仅作用于各包 src，测试 / 配置文件不进 scope），抓 `no-floating-promises`（现状 0）/ `no-misused-promises` / `no-unsafe-*` / `switch` 穷尽 / `base-to-string` 等只有类型信息才能发现的问题。`require-await` 与 `unbound-method` 因本项目函数型依赖注入风格（mapper / factory 作对象属性传递）误报高而关闭；`no-unsafe-*` 暂设 `warn` 做棘轮（可见不阻塞，后续逐步收紧）；同批修掉首轮高价值违例（冗余 union、`base-to-string`、模板表达式、回调 fire-and-forget 标注等）（[#95](https://github.com/KisinTheFlame/kagami/pull/95)）
