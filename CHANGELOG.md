...
- feat(qq): 给 QQ App 加群文件能力——`list_group_files` / `download_group_file` / `upload_group_file` 三个子工具（[#188](https://github.com/KisinTheFlame/kagami/issues/188) 之 PR2，收官）。小镜现在能列当前群的文件与文件夹、把群文件下载进 OSS 拿 `res-N`（QQ→OSS，之后可 `read_resource` 看或 `download_resource` 落地）、把一个 OSS 资源作为文件上传到群（OSS→QQ）。上传走 napcat 通用 file resolver 的 `base64://` 自包含形态（源码核实 `upload_group_file.file` 吃 base64），napcat 不需要访问 agent 的 127.0.0.1 OSS，零跨进程可达性问题。三工具都是 QQ App 子工具（经 InvokeTool，顶层工具列表零新增），仅在当前打开的会话是群时可用（私聊/未开会话 → `NOT_IN_GROUP_CHAT`）。网关新增 `listGroupFiles`（根 `get_group_root_files` / 文件夹 `get_group_files_by_folder`）、`getGroupFileUrl`、`uploadGroupFile` 三个协议方法。下载复用与图片入站同款护栏（content-length 早拒 + 实际字节兜底 + 非 2xx + `fileMaxBytes` 上限）；非图文件（pdf/zip/docx）字节探测落 `application/octet-stream`（真实类型由 `download_resource` 时给的文件名承载）。

### Changed

- chore(napcat): `NapcatGatewayService` 端口新增群文件三方法与 `NapcatGroupFile*` 类型；`buildQqApp` 注入 `ossClient` + `fileMaxBytes` 以支撑群文件下载/上传（下载要 `putObject`、上传要按 `fileMaxBytes` 大 cap `getObject`，区别于 4 MiB 上下文 cap）。
