...
- refactor(agent): 把 QQ / 群聊相关内容从主 Agent 的 system prompt 整体下沉到 QQ App 的 `help`——QQ 只是众多 App 之一，它的平台知识与聊天行为不该常驻主 Agent 的稳定前缀。新增 `apps/agent/static/prompts/qq-app-help.hbs`，`QqApp.help()` 改为渲染它（进入 QQ 后按需披露，只落上下文尾部）。
  - 从主 system prompt 移除并迁入 QQ help 的八块：`<scene>`（什么是 QQ / QQ 群）、`<attention_model>`、`<reply_decision>`、`<social_boundary>`、`<target_selection>`、`<response_policy>`、`<message_style>`、`<anti_ai_tone>`；`<qq_message>` 消息格式说明从 `<input_format>` 抽到 QQ help 的新 `<message_format>` 块。主 system prompt 收敛为「小镜是谁 + 手机 OS 怎么用」，散落的「群里 / 群友」措辞泛化为「对话 / 眼前的内容 / 过自己的生活」。
  - `<identity>` 移除小镜的 QQ 号绑定，创造者只留名字；小镜自己的 QQ 号与创造者 QQ 号改由 QQ App help 披露（在 QQ 语境里认得出创造者）。`createAgentSystemPrompt` 随之瘦身为只收 `creatorName`，`QqApp` / `buildQqApp` 新增 `creatorName` / `creatorQQ` 依赖。
  - KV 缓存：一次性重建主 Agent 稳定前缀（更短、且从此对 QQ App 数量与聊天行为改动完全无感、更稳定），群聊行为守则改由 `help` 走消息尾部按需注入，不再让每轮请求都从零换入这段前缀。
