...
Kagami 是一个基于 pnpm workspace 的全栈 TypeScript Monorepo，当前包含十四个工作空间包：
...
- `auth/`：OAuth 全套已下沉为 `@kagami/auth` 包；agent 侧仅剩 `auth-scheduled-tasks.ts`（把 auth 的刷新 scheduler / usage cache 适配成 agent scheduler 的 `ScheduledTask`，因它咬 agent 的 scheduler 领域类型故留在 app 层）
