...
- `apps/gateway`：前门网关进程（`@kagami/gateway`，独立进程、零 `@kagami/*` 依赖；托管 `apps/web/dist` 静态资源 + `/api/*` 反向代理分流到 console/metric/agent）
- `apps/metric`：metric 领域独立进程（`@kagami/metric`，一手包办 metric 摄取 `POST /metric/record`（agent HTTP 上报）+ metric-chart 查询 4 端点；经 @kagami/persistence 共享 DAO 直读 SQLite，仅 localhost）
...
- `packages/config`：配置读取的零依赖叶子包（`@kagami/config`，repo-root 定位 + `config.yaml` / `config.secret.yaml` 两文件深合并；kernel / gateway / oss / `scripts/read-config.mjs` 都复用它）
...
pnpm app:deploy <agent|console|gateway|oss|browser|metric> # 单服务：只重建并重载该服务，不跑迁移、不动其它进程
```
...
- 关键配置分区包括：
  ...
  - 顶层 `services`（与 `server` 平级）：各服务监听端口与地址的唯一事实来源，`services.{agent,console,gateway,oss,browser,metric}.{host,port}`，所有进程读它寻址（见 issue #162）；`services.browser`、`services.metric` 仅 localhost（`metric` 端口 20009，20008 已被 TEI embedding 占用）
...
- `llm/`：LLM 的 app 级装配与观测面残留——playground（HTTP handler + service）、auth-store 适配器（把 agent 的 OAuth service 适配成 `@kagami/llm-client` 的凭据端口）、embedding 缓存的 Prisma 适配器。chat client / provider / embedding client 本体已下沉到 `@kagami/llm-client`
- `napcat/`：NapCat 协议适配（gateway transport、入站归一化、图片分析、持久化写入）；网关实例由 QQ App 持有，只是 Agent 的一种事件源
- `metric/`：metric 打点的 HTTP 上报客户端（`HttpMetricService`，fire-and-forget POST 到独立 `apps/metric` 的 `/metric/record`）；摄取本体与 metric-chart 查询已下沉到 `@kagami/metric`
...
- 生产环境中，PM2 托管的网关进程 `kagami-gateway`（`apps/gateway`）会提供 `apps/web/dist`，并按路径前缀分流 `/api/*`：`/app-log`、`/llm-chat-call`、`/napcat-event`、`/napcat-group-message` 这几类纯查询代理到管理台后端进程 `kagami-console`（默认 `20006`），`/metric-chart` 代理到 metric 进程 `kagami-metric`（默认 `20009`），其余仍代理到 `kagami-agent`（默认 `20003`）。上游地址不再走 env，由网关自读 `config.yaml` 顶层 `services` 块寻址（见 issue #162）。
- 该静态服务还暴露 `/health`。
...
- 管理台后端（`kagami-console`）：单进程运行 `apps/console/dist/index.js`，默认监听 `20006`（端口取 `config.yaml` 的 `services.console.port`）。服务前端的纯 DB 查询（app-log / llm-chat-call / napcat-event / napcat-group-message），经 `@kagami/persistence` 的共享 DAO 直读同一个 SQLite 库；与 `kagami-agent` 并发读写靠库文件级 WAL（两进程启动均 `configureSqlite`）。
- 网关（`kagami-gateway`）：单进程运行 `apps/gateway/dist/index.js`（TS，零 `@kagami/*` 依赖），默认监听 `20004`，托管 `apps/web/dist` 静态资源并按前缀把 `/api/*` 分流到 `kagami-console` / `kagami-metric` / `kagami-agent`。监听端口与上游地址全部自读 `config.yaml` 的 `services` 块。
- metric 进程（`kagami-metric`）：单进程运行 `apps/metric/dist/index.js`，默认监听 `20009`（仅 localhost，端口取 `config.yaml` 的 `services.metric.port`）。一手包办 metric 摄取（`POST /metric/record`，agent 以 fire-and-forget HTTP 上报）+ metric-chart 查询（4 端点，gateway 分流至此）。经 `@kagami/persistence` 共享 DAO 直读同一个 SQLite 库（写 `metric` 表 + 读 `metric_chart`），启动 `configureSqlite` 开 WAL 与 agent/console 并发。metric 服务挂掉时打点静默丢弃、不影响 agent。
...
- `pnpm app:deploy <agent|console|gateway|oss|browser|metric>`（带服务名）只重建并重载该服务（含其依赖包），不跑迁移、不动其它进程。改了某个服务时用它即可——尤其重载 `console` / `gateway` / `browser` / `metric` 不会打断 `kagami-agent` 的热状态（KV 缓存前缀、HNSW 索引、活内存上下文），符合 KV 缓存优先原则。涉及 DB schema 变更仍走无参 `pnpm app:deploy`。（`web` 作为 `gateway` 的已弃用别名仍可用。）
