...
| `@kagami/llm-service`        | LLM 网关 + OAuth 凭据中心进程（`apps/llm`，仅 localhost）：持有全部 provider + OAuth callback server + 刷新 timer；#539 起独占 `data/llm/llm.db`（llm*chat_call / embedding_cache / claude_file_cache / oauth*\* + 自带 retention），console 的 llm-chat-call 查询经 `@kagami/llm-api` 契约路由；agent 经 HTTP 直连                                                                                                                                                                                                                                                                                          |
...
- `apps/llm` 自带独立的 `data/llm/llm.db`（自有 Prisma schema `apps/llm/prisma/`，#539）：llm_chat_call / embedding_cache / claude_file_cache / oauth_session / oauth_state 五表随进程独占，console 查询走 `@kagami/llm-api` 契约路由。
