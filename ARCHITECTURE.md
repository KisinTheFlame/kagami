...
apps/observatory ──→ packages/kernel / http / observatory-api / napcat-api / metric-client / rpc-client  （独立进程，可观测性：告警投递（去重窗口纯内存）+ 全服务日志汇聚；#608 起独占 observatory.db 持有 app_log，自有 prisma、不碰 persistence）
```
...
| `@kagami/gateway`            | 前门网关进程（独立进程，运行时仅依赖 `@kagami/config` / `@kagami/http`）：**纯反向代理，自 #578 起不再托管任何静态文件**——按前缀把 `/api/*` 分流到 console / agent，`/auth/*` + `/llm/providers` 到 llm、`/metric` 到 metric、`/oss-object` 到 oss、`/scheduler/tasks` 到 scheduler、`/gba/roms` + `/gba/console` 到 gba；`/health` 自答；其余（前端页面与静态资源）原样转给 web                                                                                                                                                                                                                             |
| `@kagami/llm-service`        | LLM 网关 + OAuth 凭据中心进程（`apps/llm`，仅 localhost）：持有全部 provider + OAuth callback server + 刷新 timer；#539 起独占 `data/llm/llm.db`（llm*chat_call / llm_blob / embedding_cache / claude_file_cache / oauth*\* + 自带 retention），console 的 llm-chat-call 查询经 `@kagami/llm-api` 契约路由；agent 经 HTTP 直连                                                                                                                                                                                                                                                                               |
...
| `@kagami/web`                | React 19 + Vite 管理台，**自 #578 起是独立进程 `kagami-web`**：`src/` 前端产物落 `dist/client`，`server/` 是自持的轻量静态服务器（产物 `dist/server`，node:http，SPA 回退 + 内容指纹长缓存），只绑 127.0.0.1、只由 gateway 反代                                                                                                                                                                                                                                                                                                                                                                              |
| `@kagami/oss`                | 自建对象存储服务（独立进程，Fastify + Prisma/SQLite；路由走 `@kagami/oss-api` 契约，get/head/delete 为 raw 路由保流式管道 / fd / 安全头语义）                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `@kagami/gba-service`        | GBA 模拟器独立进程（`apps/gba`，Fastify，仅 localhost）：内嵌 mGBA libretro WASM 核心（retroemu + romdev-platform-gba，npm pin 零运行时下载），前台=真机速率实时帧循环 / 后台=冻结，press 逐帧按键计划 + PNG 截图回传；电池存档 + ROM 元数据落自持 `data/gba/gba.db`（Prisma/SQLite），ROM 字节存 OSS；GL 原生依赖以 vendor/stubs 剥除（#541）。另透出全量内存 dump（#599）：数据源是 `retro_serialize` 快照按**实测逆向**的固定偏移切段（`retro_get_memory_data` 不可用——IWRAM 无对应 memory id，SYSTEM_RAM 声明尺寸少报 8 倍），每次 dump 用 VRAM 与 memory id 3 交叉校验布局，不符即 404 而非返回错位字节 |
| `@kagami/scheduler-service`  | 通用定时调度独立进程（`apps/scheduler`，Fastify，仅 localhost）：不认识具体业务的 cron/interval 薄时钟——使用方经 `SchedulerClient` 注册"名字+周期+补偿策略"，到点经 SSE 推 tick 回去；调度状态纯内存派生态（tick 是派生事实，断连按 misfire 合并、不做持久回放），执行历史（TaskRun）落自有 `scheduler.db`（#493）。业务逻辑（ithome/todo/data-retention）全留在 agent（#428）                                                                                                                                                                                                                               |
...
| `@kagami/observatory-api`    | kagami-observatory 进程契约包（#602 告警 + #608 日志）：`raiseAlert`（`POST /observatory/alert`）+ `ingestLogs`（`POST /observatory/logs`，批量摄取，items ≤ 500，`createdAt` 由产出进程盖戳）+ `queryLogs`（`POST /observatory/logs/query`，console 拨）。告警载荷是通用形状（source / event / severity / title / detail? / context?）——observatory 不认识任何调用方的领域词汇；回执用 `delivered` / `suppressed` 两个布尔表达三种结局，通道失败也回 200（不是调用方的错，不该重试）                                                                                                                        |
| `@kagami/observatory-client` | kagami-observatory 的上报 SDK（#608），镜像 `@kagami/metric-client`：`HttpLogSink`（实现 kernel 的 `LogSink`，攒批 → 定时 flush → 一次 HTTP，满队丢弃计数、失败即丢不重试；**失败路径只写 stderr，绝不走 AppLogger**，否则 observatory 一挂就自噬成无限循环）+ `HttpObservatoryClient`（告警，#608 起从 agent 的 ACL 搬来供全体服务共用）。住在 kernel 外面是硬约束：`rpc-client` 依赖 `kernel`，kernel 不能反向依赖它                                                                                                                                                                                       |
...
- **进程内 SQLite 文件**（agent 库默认 `data/agent/agent.db`，原名 kagami.db）通过 Prisma ORM + `@prisma/adapter-better-sqlite3` 访问；宿主机不再需要外部 PostgreSQL。
...
- `apps/napcat` 自带独立的 `data/napcat/napcat.db`（自有 Prisma schema `apps/napcat/prisma/`，#539）：napcat_event / napcat_qq_message / napcat_event_outbox / image_asset 四表随进程独占，console 查询走 `@kagami/napcat-api` 契约路由。
- `apps/llm` 自带独立的 `data/llm/llm.db`（自有 Prisma schema `apps/llm/prisma/`，#539）：llm_chat_call / llm_blob / embedding_cache / claude_file_cache / oauth_session / oauth_state 六表随进程独占，console 查询走 `@kagami/llm-api` 契约路由。其中 `llm_blob` 是 `llm_chat_call` 请求体的内容寻址分片存储（#612）：ReAct 每轮重发全部历史，逐轮存完整副本让单会话累计写入呈 O(轮数²)（生产实测 23104 条 message 引用里只有 11% 唯一），故 `messages` 逐条按未压缩字节的 sha256 落 blob（zstd），`llm_chat_call` 行内只留 `request_skeleton` 与 packed Uint32 的 `message_refs`，写入降到 O(轮数)。对外 `requestPayload` 形状零变更，`findById` 在读侧重组还原。
...
- PM2（`ecosystem.config.cjs`）托管以下进程：`kagami-agent`（Fastify，Agent 运行时 + 活内存接口，默认 20003）、`kagami-console`（管理台后端，前端只读查询聚合，#539 起零 DB 依赖，默认 20006）、`kagami-gateway`（`apps/gateway`，纯反代：按前缀把 `/api/*` 分流到 console/agent、`/auth/*` 到 llm、`/metric/query` 到 metric，其余转 `kagami-web`，默认 20004）、`kagami-web`（`apps/web`，管理台前端独立进程，自持静态托管，默认 20016，仅 localhost；#578 起从 gateway 拆出，前端产物不再于构建期装配进网关，两者生命周期独立——`app:deploy web` 不动网关、`app:deploy gateway` 不动前端）、`kagami-oss`（对象存储，默认 20005，仅 localhost）、`kagami-browser`（`apps/browser`，持有 CloakBrowser，默认 20007，仅 localhost；`cwd` 固定仓库根，agent 重启不杀浏览器，`app:deploy agent` 不触及它，见 #173；#539 删 `browser_credential` 废表后零持久化、不开任何 SQLite）、`kagami-llm`（`apps/llm`，LLM + OAuth 凭据网关，默认 20009，仅 localhost；持有 provider + callback server + 刷新 timer + 经 `SchedulerClient` 跑两个每日 GC（Claude Files 缓存按 `last_used_at` idle 回收远端文件 + 本地行，#433；`llm_blob` 孤儿分片 mark-sweep 回收，排在 `data-retention:llm_chat_call` 之后，同样按 `last_used_at` 设宽限窗口——命中复用即续期，挡住「mark 之后 sweep 之前被重新引用」的悬挂引用窗口，#612），`app:deploy agent` 不触及它；#539 起独占 `data/llm/llm.db`（自带 retention 清理），不碰主库；llm 库迁移由 deploy.sh 单停 kagami-llm 执行）、`kagami-metric`（`apps/metric`，metric 摄取 + metric 图表查询，默认 20010，仅 localhost；agent fire-and-forget HTTP 上报；#475 P1 起独占 `data/metric/metric.duckdb`，不开共享 SQLite，主库迁移不需停它）、`kagami-spire`（`apps/spire`，杀塔式卡牌游戏引擎，默认 20011，仅 localhost；`cwd` 固定仓库根让存档 `data/spire/` 落仓库根，`app:deploy agent` 不触及它，agent 重启不打断对局，见 #234）、`kagami-pixel`（`apps/pixel`，像素画渲染引擎，默认 20012，仅 localhost；`cwd` 固定仓库根让存档 `data/pixel/` 落仓库根，`app:deploy agent` 不触及它，agent 重启不丢画布，见 #365）、`kagami-napcat`（`apps/napcat`，NapCat 接入独立进程，默认 20013，仅 localhost；持有到 NapCat 的 WS 长连接 + 出站 RPC + 入站 SSE + vision/OSS 存档 + napcat 表落库，`cwd` 固定仓库根，`app:deploy agent` 不触及它，agent 重启不打断 QQ 连接，见 #347；#539 起独占 `data/napcat/napcat.db`（napcat_event / napcat_qq_message / outbox / image_asset + 自带 retention 清理），不碰主库；napcat 库迁移由 deploy.sh 单停 kagami-napcat 执行）、`kagami-scheduler`（`apps/scheduler`，通用定时调度薄时钟，默认 20014，仅 localhost；调度状态纯内存派生态、执行历史独占 `data/scheduler/scheduler.db`（#493），agent 与 kagami-llm 经 `SchedulerClient` 注册 + SSE 收 tick，`app:deploy agent` 不触及它，agent 重启不打断计时节奏，见 #428）、`kagami-gba`（`apps/gba`，GBA 模拟器独立进程，默认 20015，仅 localhost；内嵌 mGBA WASM 核心，前台实时帧循环 / 后台冻结，`cwd` 固定仓库根让 `data/gba/`（ROM 元数据 + 电池存档 sqlite）落仓库根，ROM 字节存 OSS，`app:deploy agent` 不触及它，agent 重启不丢模拟器热状态与存档，见 #541；优雅关停落 savestate 快照（`resume_state` 单行，消费即删），`app:deploy gba` 重启后 unserialize 接续停机现场（含前后台）实现无感重启，agent 侧 gba-client 另有连接被拒短重试吃掉重启窗口；crash 无快照，落回「断电 + 电池存档」语义）、`kagami-observatory`（`apps/observatory`，告警能力独立进程，默认 20017，仅 localhost；任何服务经 `POST /observatory/alert` 上报，observatory 渲染 + 按 `(source, event)` 5 分钟窗口去重限流后经 napcat 投递到专用 QQ 告警群，见 #602；自 #608 起还是**全服务日志汇聚点**：各进程经 `POST /observatory/logs` 批量上报，落独占 `data/observatory/observatory.db` 的 `app_log`（进程内 1h prune，保留 7 天），console 经 `POST /observatory/logs/query` 查回给管理台；observatory 自身日志走本地 `DbLogSink` 直写、绝不 HTTP 打回自己；告警去重窗口仍是纯内存，重启清零，最坏是重启后多发一条重复告警；`cwd` 固定仓库根，`app:deploy agent` 不触及它，agent 重启不打断告警通道、observatory 挂掉也不影响 agent（上报是 fire-and-forget，上报方本地 `logger.error` 兜底留痕）；已知单点：投递链路依赖 napcat，napcat 挂掉时告警发不出去——第二通道待「长成可观测性平台」那一步）。agent 库 `data/agent/agent.db`（原主库 kagami.db）自 #539 起由 **kagami-agent 独占**（console 零 DB、经各服务查询路由聚合；`apps/spire` / `apps/pixel` / `apps/scheduler` / `apps/metric` / `apps/browser` / `apps/gba` / `apps/napcat` / `apps/llm` 不入共享库，spire/pixel 存档走 JSON、scheduler 独占 `data/scheduler/scheduler.db`、metric 独占 DuckDB、browser 零持久化 #539、gba 自持 `data/gba/gba.db`、napcat 独占 napcat.db #539、llm 独占 llm.db #539、observatory 独占 observatory.db #608）。`app_log` 自 #608 起不在 agent 库——全服务日志归 kagami-observatory。
