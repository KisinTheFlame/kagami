...
| `@kagami/console`           | 管理台后端独立进程（Fastify），服务前端查询聚合：app-log / llm-chat-call / inner-thought / todo 经 @kagami/persistence 共享 DAO 直读主库 SQLite；napcat-event / napcat-group-message 自 #539 起经 `@kagami/napcat-api` 查询路由拨 kagami-napcat（不再直读）                                                                                    |
| `@kagami/gateway`           | 前门网关进程（独立进程，运行时仅依赖 `@kagami/config` / `@kagami/http`；构建期 devDep `@kagami/web`，build 期把其 dist 装配进自身 `dist/public`，运行时只读自身目录、不跨 app 伸手，#496）：托管自身 `dist/public` 前端静态资源 + 按前缀把 `/api/*` 反代分流到 console / agent，`/auth/*` + `/llm/providers` 到 llm、`/metric/query` 到 metric |
