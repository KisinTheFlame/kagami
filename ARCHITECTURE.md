...
| `@kagami/console`           | 管理台后端独立进程（Fastify），服务前端查询聚合：app-log / inner-thought / todo 经 @kagami/persistence 共享 DAO 直读主库 SQLite；napcat 两表经 `@kagami/napcat-api`、llm-chat-call 经 `@kagami/llm-api` 查询路由拨各自服务（#539，不再直读）                                                                                                                                                                                             |
...
- `apps/napcat` 自带独立的 `data/sqlite/napcat.db`（自有 Prisma schema `apps/napcat/prisma/`，#539）：napcat_event / napcat_qq_message / napcat_event_outbox / image_asset 四表随进程独占，console 查询走 `@kagami/napcat-api` 契约路由；主库中的同名旧表待 #539 子 issue 5 统一 DROP。
- `apps/llm` 自带独立的 `data/sqlite/llm.db`（自有 Prisma schema `apps/llm/prisma/`，#539）：llm_chat_call / embedding_cache / claude_file_cache / oauth_session / oauth_state 五表随进程独占，console 查询走 `@kagami/llm-api` 契约路由；主库旧表同待子 issue 5 DROP。
...
- PM2（`ecosystem.config.cjs`）托管以下进程：`kagami-agent`（Fastify，Agent 运行时 + 活内存接口，默认 20003）、`kagami-console`（管理台后端，服务前端纯 DB 查询，默认 20006）、`kagami-gateway`（`apps/gateway`，静态 + 按前缀把 `/api/*` 分流到 console/agent、`/auth/*` 到 llm、`/metric/query` 到 metric，默认 20004）、`kagami-oss`（对象存储，默认 20005，仅 localhost）、`kagami-browser`（`apps/browser`，持有 CloakBrowser，默认 20007，仅 localhost；`cwd` 固定仓库根，agent 重启不杀浏览器，`app:deploy agent` 不触及它，见 #173；#539 删 `browser_credential` 废表后零持久化、不开共享 SQLite——删表迁移本身在生产落地后，主库迁移即不再需要停它，届时从 deploy.sh 停服名单移除）、`kagami-llm`（`apps/llm`，LLM + OAuth 凭据网关，默认 20009，仅 localhost；持有 provider + callback server + 刷新 timer + 经 `SchedulerClient` 跑 Claude Files 缓存每日 GC（`claude_file_cache` 按 `last_used_at` idle 回收远端文件 + 本地行，#433），`app:deploy agent` 不触及它；#539 起独占 `data/sqlite/llm.db`（自带 retention 清理），不再长期打开主库——历史数据启动时一次性幂等搬迁，llm 库迁移由 deploy.sh 单停 kagami-llm 执行）、`kagami-metric`（`apps/metric`，metric 摄取 + metric 图表查询，默认 20010，仅 localhost；agent fire-and-forget HTTP 上报；#475 P1 起独占 `data/metric/metric.duckdb`，不开共享 SQLite，主库迁移不需停它）、`kagami-spire`（`apps/spire`，杀塔式卡牌游戏引擎，默认 20011，仅 localhost；`cwd` 固定仓库根让存档 `data/spire/` 落仓库根，`app:deploy agent` 不触及它，agent 重启不打断对局，见 #234）、`kagami-pixel`（`apps/pixel`，像素画渲染引擎，默认 20012，仅 localhost；`cwd` 固定仓库根让存档 `data/pixel/` 落仓库根，`app:deploy agent` 不触及它，agent 重启不丢画布，见 #365）、`kagami-napcat`（`apps/napcat`，NapCat 接入独立进程，默认 20013，仅 localhost；持有到 NapCat 的 WS 长连接 + 出站 RPC + 入站 SSE + vision/OSS 存档 + napcat 表落库，`cwd` 固定仓库根，`app:deploy agent` 不触及它，agent 重启不打断 QQ 连接，见 #347；#539 起独占 `data/sqlite/napcat.db`（napcat_event / napcat_qq_message / outbox / image_asset + 自带 retention 清理），不再长期打开主库——历史数据启动时一次性幂等搬迁，napcat 库迁移由 deploy.sh 单停 kagami-napcat 执行）、`kagami-scheduler`（`apps/scheduler`，通用定时调度薄时钟，默认 20014，仅 localhost；无 DB、纯内存派生态，agent 与 kagami-llm 经 `SchedulerClient` 注册 + SSE 收 tick，`app:deploy agent` 不触及它，agent 重启不打断计时节奏，见 #428）、`kagami-gba`（`apps/gba`，GBA 模拟器独立进程，默认 20015，仅 localhost；内嵌 mGBA WASM 核心，前台实时帧循环 / 后台冻结，`cwd` 固定仓库根让 `data/gba/`（ROM 元数据 + 电池存档 sqlite）落仓库根，ROM 字节存 OSS，`app:deploy agent` 不触及它，agent 重启不丢模拟器热状态与存档，见 #541）。后端进程并发读写同一 SQLite 库靠库文件级 WAL——共享主库的进程仅剩 agent 与 console（`apps/spire` / `apps/pixel` / `apps/scheduler` / `apps/metric` / `apps/browser` / `apps/gba` / `apps/napcat` / `apps/llm` 不入共享库，spire/pixel 存档走 JSON、scheduler 独占 scheduler.db、metric 独占 DuckDB、browser 零持久化 #539、gba 自持 `data/gba/gba.db`、napcat 独占 napcat.db #539、llm 独占 llm.db #539）。
