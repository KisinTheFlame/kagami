...
| `@kagami/gba-service`       | GBA 模拟器独立进程（`apps/gba`，Fastify，仅 localhost）：内嵌 mGBA libretro WASM 核心（retroemu + romdev-platform-gba，npm pin 零运行时下载），前台=真机速率实时帧循环 / 后台=冻结，press 逐帧按键计划 + PNG 截图回传；电池存档 + ROM 元数据落自持 `data/gba/gba.db`（Prisma/SQLite），ROM 字节存 OSS；GL 原生依赖以 vendor/stubs 剥除（#541）                                                                                           |
