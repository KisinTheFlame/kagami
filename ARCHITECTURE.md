...
apps/observatory ──→ packages/kernel / http / observatory-api / napcat-api / metric-client / rpc-client  （独立进程，告警能力；零 DB，去重窗口纯内存，投递经 napcat 发 QQ 告警群）
```
...
| `@kagami/observatory-api`   | kagami-observatory 进程契约包（#602）：单条 `raiseAlert` JSON 路由（`POST /observatory/alert`）+ `AlertSeverity`（warn/error/fatal，对齐 kernel LogLevel 词汇）。载荷是通用告警形状（source / event / severity / title / detail? / context?）——observatory 不认识任何调用方的领域词汇；回执用 `delivered` / `suppressed` 两个布尔表达三种结局，通道失败也回 200（不是调用方的错，不该重试）                                                                                                                                                                                                                  |
