...
apps/gateway ──→ packages/config  （独立进程，零 @kagami/* 运行时依赖，静态托管 + /api 反向代理）
apps/oss     ──→ packages/config  （独立进程，零 @kagami/* 运行时依赖，自建对象存储）
apps/browser ──→ packages/persistence / kernel / http  （独立进程，持有 CloakBrowser）
```

> `packages/config` 是零依赖叶子（repo-root 定位 + 两文件深合并），被 kernel / gateway / oss 复用。
...
| `@kagami/metric`        | metric 领域独立进程（Fastify），一手包办 metric 摄取（`POST /metric/record`，agent HTTP 上报）+ metric-chart 查询，经 @kagami/persistence 共享 DAO 直读 SQLite，仅 localhost          |
| `@kagami/gateway`       | 前门网关独立进程（`node:http`，零 `@kagami/*` 运行时依赖，仅 `@kagami/config`）：托管 `apps/web/dist` + 按前缀把 `/api/*` 反向代理分流到 console / metric / agent                     |
...
| `@kagami/config`        | 配置读取的零依赖叶子（repo-root 定位 + `config.yaml` / `config.secret.yaml` 深合并；kernel / gateway / oss 复用）                                                                     |
...
| `@kagami/llm-client`    | LLM chat client + provider + embedding client 运行时（位于 kernel 之上、与 persistence 平级且互不依赖；只发 `LlmChatCallObservation` 事件，落库 / 缓存归 agent 装配层订阅）           |
...
- PM2（`ecosystem.config.cjs`）托管六个进程：`kagami-agent`（Fastify，Agent 运行时 + 活内存接口，默认 20003）、`kagami-console`（管理台后端，服务前端纯 DB 查询，默认 20006）、`kagami-gateway`（`apps/gateway`，静态 + 按前缀把 `/api/*` 分流到 console/metric/agent，默认 20004）、`kagami-metric`（`apps/metric`，metric 摄取 + metric-chart 查询，默认 20009，仅 localhost）、`kagami-oss`（对象存储，默认 20005，仅 localhost）、`kagami-browser`（`apps/browser`，持有 CloakBrowser，默认 20007，仅 localhost；`cwd` 固定仓库根，agent 重启不杀浏览器，`app:deploy agent` 不触及它，见 #173）。后端进程并发读写同一 SQLite 库靠库文件级 WAL。
