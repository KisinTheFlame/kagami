...
| `@kagami/pixel-api`     | kagami-pixel 进程的像素画 RPC 契约包（#365）：9 条 JSON 绘图/查看路由回 `CanvasResponse`（领域拒绝走 `{ok:false}`）+ render binary-raw 路由回 PNG 字节；含 DB16 命名调色板（name/glyph/hex）共享常量                                                                                                                 |
