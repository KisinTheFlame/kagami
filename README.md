...
Kagami is a full-stack TypeScript monorepo built on `pnpm workspace`, currently containing fifteen workspace packages:
...
- `apps/gateway`: front-door gateway process (`@kagami/gateway`, standalone with zero `@kagami/*` dependencies; serves the `apps/web/dist` static assets and reverse-proxies `/api/*` to console/metric/agent)
...
- The PM2 config file is [ecosystem.config.cjs](./ecosystem.config.cjs). It manages six processes.
...
- The metric service `kagami-metric` runs `apps/metric/dist/index.js` and listens on `20009` by default (localhost only); it owns metric ingestion (`POST /metric/record`, the agent reports fire-and-forget over HTTP) plus the metric-chart query endpoints, which the gateway routes to it.
