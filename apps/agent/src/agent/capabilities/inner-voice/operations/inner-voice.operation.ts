...
/**
 * 内心独白 Operation（issue #265）：隔离 LLM 调用，以小镜口吻产出一个锚定近期真实
 * 经历、指向一步可达可供性的第一人称念头。结构对称 ContextSummaryOperation——
 * toolChoice 强制指到 emit_inner_thought，空 thought 即「没念头」。
 *
 * 大量素材（尾部切片）只进这次隔离调用，主 Agent 前缀零污染；产出至多一句话经
 * InnerThoughtEvent 回流。
 */
export class InnerVoiceOperation implements Operation<InnerVoiceInput, InnerVoiceResult> {
  ...
  public async execute(input: InnerVoiceInput): Promise<InnerVoiceResult> {
    ...
    const thought = truncateByCodePoints(executionResult.content.trim(), MAX_THOUGHT_CODE_POINTS);

    return { thought: thought.length > 0 ? thought : null };
  }
}

/** 按 Unicode 码点截断，绝不把代理对劈成 lone surrogate（教训见 issue #187）。 */
