...
export async function buildAgentRuntime({
  ...
  const emitInnerThoughtTool = new EmitInnerThoughtTool();
  ...
  // emit_inner_thought 只作为 Operation 的结构化出口，不进主 Agent 工具集。
  const innerVoiceIdleTracker = new InnerVoiceIdleTracker();
  const innerVoiceOperation = new InnerVoiceOperation({
    llmClient,
    emitToolExecutor: toolCatalog.pick([EMIT_INNER_THOUGHT_TOOL_NAME]),
