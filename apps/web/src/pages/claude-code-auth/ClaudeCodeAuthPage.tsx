...
export function ClaudeCodeAuthPage() {
  ...
  const statusQuery = useQuery({
    queryKey: ["claude-code-auth-status"],
    queryFn: async () => {
      const response = await apiFetch<unknown>("/claude-code-auth/status");
      return ClaudeCodeAuthStatusResponseSchema.parse(response);
    },
  });

  const loginMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("/claude-code-auth/login-url", {
        method: "POST",
        body: JSON.stringify({}),
      });
      if (!response.ok) {
      ...
      return ClaudeCodeAuthLoginUrlResponseSchema.parse(response.body);
    },
    onSuccess: data => {
  ...
  const refreshMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("/claude-code-auth/refresh", {
        method: "POST",
        body: JSON.stringify({}),
      });
      if (!response.ok) {
      ...
      return ClaudeCodeAuthRefreshResponseSchema.parse(response.body);
    },
    onSuccess: async () => {
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("/claude-code-auth/logout", {
        method: "POST",
        body: JSON.stringify({}),
      });
      if (!response.ok) {
  ...
  return (
    <div className="flex h-full min-h-0 w-full min-w-0 flex-col overflow-auto bg-[radial-gradient(circle_at_top_right,_rgba(245,158,11,0.12),_transparent_24%),radial-gradient(circle_at_bottom_left,_rgba(14,165,233,0.12),_transparent_28%),linear-gradient(180deg,_rgba(248,250,252,0.98),_rgba(241,245,249,0.88))] p-3 md:p-6">
        ...
        <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <article className="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-sm">
            ...
            {statusQuery.isLoading ? (
              <p className="mt-6 text-sm text-slate-500">正在读取 Claude Code 登录状态...</p>
            ) : statusQuery.isError ? (
          ...
          <article className="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-500">
              首版按单账号设计。登录会跳转到 Anthropic 的授权页，成功后回到当前管理页。
            </p>
