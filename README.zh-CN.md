...
- `apps/gateway`：前门网关进程（`@kagami/gateway`，独立进程、零 `@kagami/*` 依赖；托管 `apps/web/dist` 静态资源 + `/api/*` 反向代理分流到 console/agent、`/auth/*` 到 llm、`/metric-chart` 与 `/observability` 到 metric）
...
- `apps/metric`：metric 领域独立进程（`@kagami/metric`，一手包办 metric 摄取 `POST /metric/record`（agent HTTP 上报）+ metric-chart 查询 + `/observability/*` LLM 观测查询（直连聚合 `llm_chat_call`）；经 @kagami/persistence 共享 DAO 直读同一 SQLite，仅 localhost）
...
apps/
  ...
  metric/   metric 摄取 + metric-chart 查询 + LLM 观测查询（独立进程）
  oss/      自建内容寻址对象存储（独立进程）
...
- `capabilities/`：按能力聚合的实现，当前包括 `messaging`、`context-summary`、`ledger`、`ithome`、`vision`、`web-search`、`browser`、`resource`、`spire`、`terminal`、`todo`、`inner-voice`
...
- metric 服务 `kagami-metric` 运行 `apps/metric/dist/index.js`，默认监听 `20010`（仅 localhost）；一手包办 metric 摄取（`POST /metric/record`，agent fire-and-forget HTTP 上报）+ metric-chart 查询 + `/observability/*` LLM 观测查询（网关分流至此）。
