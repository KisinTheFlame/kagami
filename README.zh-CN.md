...
- 修改配置 schema 时，必须同步更新：
  ...
  - `config.yaml`
  - `config.yaml.example`
- `server.llm.usages` 需要完整提供 `agent`、`storyAgent`、`contextSummarizer`、`vision`、`webSearchAgent` 等尝试链路。
...
- 当前核心导出包括 `TaskAgent`、`Operation`、`App` / `AppManager` / `AppStateStore` 框架、`InvokeTool`、`ToolCatalog`、`ToolSet`、`ToolExecutor` 等抽象。
