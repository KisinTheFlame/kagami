...
Kagami 是一个基于 `pnpm workspace` 的全栈 TypeScript Monorepo，当前包含十七个工作空间包：
...
- `capabilities/`：按能力聚合的实现，当前包括 `messaging`、`context-summary`、`ledger`、`ithome`、`vision`、`web-search`、`browser`、`terminal`、`todo`、`resource`
...
- PM2 配置文件位于 [ecosystem.config.cjs](./ecosystem.config.cjs)，托管七个进程。
